...
const DEFAULT_WIDGET_STATES = {
  ...
  curationActions: { visible: false, minimized: false, maximized: false, order: 20, position: null, size: null, type: 'curationActions' }
};
...
const MultiPanelView = forwardRef(({
  ...
  const handleCurationSpikeSelection = useCallback((selection) => {
    const normalized = [...new Map((Array.isArray(selection) ? selection : [])
      .filter((spike) => spike?.spikeId)
      .map((spike) => [spike.spikeId, spike])).values()];
