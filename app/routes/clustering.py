...
def _ensure_requested_clustering_results(clustering_manager, algorithm=''):
    """Keep the singleton manager aligned with the algorithm requested by a view."""
    source = getattr(clustering_manager, 'clustering_results_source', None)
    results_missing = clustering_manager.clustering_results is None

    if algorithm == 'preprocessed_torchbci' and (results_missing or source != algorithm):
        clustering_manager.load_preprocessed_torchbci()
    elif algorithm == 'preprocessed_kilosort4' and (results_missing or source != algorithm):
...
def _load_clustering_results(algorithm=''):
    ...
    _ensure_requested_clustering_results(clustering_manager, algorithm)
